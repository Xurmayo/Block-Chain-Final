...
window.confirmNFTTrade = async function() {
  ...
  try {
    ...
    // Inform user about two transactions
...
async function loadNFTs() {
  ...
  // Show "Set NFT as Avatar" button if user owns at least one NFT
  el("setNFTBtn").style.display = hasNFT ? "inline-block" : "none";
  // If user owns NFTs and none selected, select the first one and set as avatar
  if (hasNFT && !selectedNFT) {
...
window.setNFTAsAvatar = function() {
  if(selectedNFT) {
    ...
    // Optionally, refresh NFT list to keep highlight
    loadNFTs();
  }
};
