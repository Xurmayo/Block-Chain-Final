...
async function login(r) {
  ...
  // Use signer for tokenContract so user can burn their own tokens
  try {
  ...
  await loadNFTs();
  await displayBadge(); // Show badge after login
...
window.confirmNFTTrade = async function() {
  ...
  try {
    ...
    // Burn 10 CTKN (must use signer)
    await tokenContract.burn(userAddress, burnAmount);
    // Mint NFT (must use signer)
    const uri = NFT_CHOICES[selectedNFTChoice].uri;
    await nftContract.mint(userAddress, uri);
    hideNFTChoices();
    await loadNFTs();
    await updateBalance();
    await displayBadge(); // Show badge after claim
  } catch (err) {
...
// Display the first NFT badge as the user's badge (next to avatar)
async function displayBadge() {
  try {
    ...
    if (count > 0) {
      ...
      const uri = await nftContract.tokenURI(tokenId);
      // Show badge next to avatar
      el("avatar").innerHTML += ` <img src="${uri}" style="border-radius:50%;vertical-align:middle;width:32px;height:32px;border:2px solid #22c55e;margin-left:4px;" title="Your NFT Badge">`;
    }
  } catch (err) {
    // No badge or error, do nothing
  }
}
