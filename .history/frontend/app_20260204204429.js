...
async function submitCampaign(){
  try {
    ...
    el("goal").value = "";
    el("duration").value = "";
    // Wait a bit before loading campaigns to let the chain index update
    setTimeout(() => loadCampaigns(true), 1200);
  } catch (err) {
...
async function loadCampaigns(forceRender = false){
  ...
  let count = 0;
  try {
    // Fix: Always convert to Number from BigInt
    count = Number(await contract.campaignCount());
  } catch (e) {
    console.error("Error fetching campaignCount:", e);
    // Do not clear UI if error
    return;
  }

  // Defensive: If count is 0 but we had campaigns, do not clear UI unless it's the first load
  if (count === 0 && campaignsCache.length > 0 && !forceRender) {
    console.warn("Campaign count is 0 but cache is not empty. Skipping UI clear.");
    return;
  }
  ...
  // Only update cache and render if we actually got campaigns or it's the first load
