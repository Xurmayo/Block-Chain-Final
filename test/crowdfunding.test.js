...
describe("Crowdfunding basic flows", function () {
  ...
  it("rejects submitCampaign with zero duration", async () => {
    await expect(
      crowdfunding.connect(creator).submitCampaign("Bad", "", ethers.parseEther("1"), 0)
    ).to.be.reverted;
  });

  it("only moderator can approve campaign", async () => {
    await crowdfunding.connect(creator).submitCampaign("T3", "", ethers.parseEther("1"), 100);
    await expect(
  ...
  it("only moderator can reject campaign", async () => {
    await crowdfunding.connect(creator).submitCampaign("T4", "", ethers.parseEther("1"), 100);
    await expect(
  ...
  it("only creator can withdraw from successful campaign", async () => {
    await crowdfunding.connect(creator).submitCampaign("T5", "", ethers.parseEther("1"), 1000);
    await crowdfunding.connect(deployer).approveCampaign(0);
    await crowdfunding.connect(contributor).contribute(0, { value: ethers.parseEther("1") });
    await expect(
  ...
  it("tracks campaign contributors and contribution amounts", async () => {
    await crowdfunding.connect(creator).submitCampaign("T6", "", ethers.parseEther("10"), 1000);
    await crowdfunding.connect(deployer).approveCampaign(0);
    await crowdfunding.connect(contributor).contribute(0, { value: ethers.parseEther("2") });
    await crowdfunding.connect(other).contribute(0, { value: ethers.parseEther("3") });
  ...
  it("contribute requires active campaign and reverts for rejected", async () => {
    await crowdfunding.connect(creator).submitCampaign("T7", "", ethers.parseEther("1"), 100);
    await crowdfunding.connect(deployer).rejectCampaign(0);
    await expect(
      crowdfunding.connect(contributor).contribute(0, { value: ethers.parseEther("0.5") })
    ).to.be.revertedWith("Not active");
  });

  it("refund reverts when campaign is not failed", async () => {
    await crowdfunding.connect(creator).submitCampaign("T8", "", ethers.parseEther("5"), 2);
    await crowdfunding.connect(deployer).approveCampaign(0);
    await crowdfunding.connect(contributor).contribute(0, { value: ethers.parseEther("1") });
    await expect(
  ...
  it("campaignCount returns correct number of campaigns", async () => {
    ...
    await crowdfunding.connect(creator).submitCampaign("A", "", ethers.parseEther("1"), 100);
    expect(await crowdfunding.campaignCount()).to.equal(1);
    await crowdfunding.connect(creator).submitCampaign("B", "", ethers.parseEther("1"), 100);
